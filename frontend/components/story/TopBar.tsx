...
export const PITCH_URL = "https://youtu.be/J_Q-N8HDrGA";
...
/** YouTube play mark (monochrome, matches the token palette). */
function YouTubeMark({ size = 15 }: { size?: number }) {
  return (
        ...
        {section === 0 && (
          <>
            <a
              ...
              title="Watch the 2-minute pitch"
              className="relative flex items-center gap-2 rounded-full border
                border-accent/40 bg-accent/10 px-4 py-2 text-xs font-medium
                text-accent transition hover:bg-accent/20"
            >
              {/* flashing halo */}
              <span
                aria-hidden
              ...
              />
              <YouTubeMark />
              <span className="hidden sm:block">View Pitch</span>
            </a>
            <a
              href={REPO_URL}
              target="_blank"
